...
const RelatedDoctors = ({ department, docId }) => {
    ...
    useEffect(() => {
        if (doctors.length > 0 && department) {
            const doctorsData = doctors.filter((doc) => doc.department === department && doc._id !== docId)
    ...
    return (
        <div className='flex flex-col items-center gap-4 my-16 text-[#262626]'>
            <h1 className='text-3xl font-medium'>Related Staff</h1>
            <p className='sm:w-1/3 text-center text-sm'>Other staff members in the same department.</p>
